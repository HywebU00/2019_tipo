<!doctype html>
<html lang="zh-Hant" class="no-js">

<head>
<meta name="robots" content="noindex">
<!-- 使用 noindex 禁止 Google 搜尋建立索引，套版時請工程師刪掉這行 -->
  <meta charset="utf-8">
  <meta http-equiv="X-UA-Compatible" content="IE=edge">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>經濟部智慧財產局</title>
    <!--HTML5 Shim and Respond.js IE8 support of HTML5 elements and media queries [if lt IE 9]>
    <script src="js/html5shiv.js"></script>
    <script src="js/respond.min.js"></script>
  <![endif]-->
  <!-- slick css-->
  <link rel="stylesheet" type="text/css" href="vendor/slick/slick.css" />
  <link rel="stylesheet" type="text/css" href="vendor/slick/slick-theme.css" />
  <!-- hyUI css -->
  <link rel="stylesheet" href="css/hyui.css">
  <!-- ico -->
  <link rel="icon" type="image/ico" href="images/favicon.ico">
</head>

<body>
  <!-- 直接跳主內容區 -->
  <a class="goCenter" href="#center" tabindex="1">按Enter到主內容區</a>
  <!-- wrapper Start -->
  <div class="wrapper">
    <!-- header Start -->
    <?php require_once('include/header.html'); ?>
    <!-- header End -->
    <!-- main Start -->
    <div id="center" class="main innerpage">
      <!-- 內頁新增一個classname-->
      <a class="accesskey" href="#aC" id="aC" accesskey="C" title="主要內容區">:::</a>
      <div class="container">
        <!-- breadcrumb路徑 -->
        <?php require_once('include/breadcrumb.html'); ?>
        <!-- h2節點 -->
        <h2 class="title">積體電路電路布局保護法規資訊-lp7混合查詢列表頁</h2>
        <!-- function功能區塊 -->
        <?php require_once('include/function.html'); ?>
        <section class="lp">
          <!-- advanced_search -->
          <div class="advanced_search_block2">
            <div class="advanced_search" style="">
              <form action="" class="form_grid">
                <div class="form_grp">
                  <label for="" class="form_title">修正日期：</label>
                  <div class="form_content">
                    <div class="form_grp form_inline">
                      <input name="" type="text" value="" id="calendarStart" placeholder="開始日期"><a href="" class="btn_calendar"></a> ~
                      <input name="" type="text" value="" id="calendarEnd" placeholder="結束日期"><a href="" class="btn_calendar"></a>
                    </div>
                  </div>
                </div>
                <div class="form_grp">
                  <label for="sameId" class="form_title">中文法規名稱：</label>
                  <div class="form_content">
                    <input type="text" id="sameId" value="" placeholder="請輸入標題">
                  </div>
                </div>
                <div class="btn_grp">
                  <input name="Search" type="submit" value="查詢">
                  <input name="" type="reset" value="重設">
                </div>
              </form>
            </div>
          </div>
          <div class="table_list">
            <table summary="">
              <thead>
                <tr>
                  <th>序號</th>
                  <th>修正日期</th>
                  <th>中文法規名稱</th>
                  <th>更新日期</th>
                </tr>
              </thead>
              <tbody>
                <tr>
                  <td data-title="序號" class="num">01</td>
                  <td data-title="修正日期" class="num">民國91年6月12日</td>
                  <td data-title="中文法規名稱" class="num">積體電路電路布局保護法</td>
                  <td data-title="更新日期" class="num">民國106年6月12日</td>
                </tr>
                <tr>
                  <td data-title="編號" class="num">02</td>
                  <td data-title="修正日期" class="num">民國91年6月12日</td>
                  <td data-title="中文法規名稱" class="num">積體電路電路布局保護法</td>
                  <td data-title="更新日期" class="num">民國106年6月12日</td>
                </tr>
                <tr>
                  <td data-title="編號" class="num">03</td>
                  <td data-title="修正日期" class="num">民國91年6月12日</td>
                  <td data-title="中文法規名稱" class="num">積體電路電路布局保護法</td>
                  <td data-title="更新日期" class="num">民國106年6月12日</td>
                </tr>
                <tr>
                  <td data-title="編號" class="num">04</td>
                  <td data-title="修正日期" class="num">民國91年6月12日</td>
                  <td data-title="中文法規名稱" class="num">積體電路電路布局保護法</td>
                  <td data-title="更新日期" class="num">民國106年6月12日</td>
                </tr>
                <tr>
                  <td data-title="編號" class="num">05</td>
                  <td data-title="修正日期" class="num">民國91年6月12日</td>
                  <td data-title="中文法規名稱" class="num">積體電路電路布局保護法</td>
                  <td data-title="更新日期" class="num">民國106年6月12日</td>
                </tr>
                <tr>
                  <td data-title="編號" class="num">06</td>
                  <td data-title="修正日期" class="num">民國91年6月12日</td>
                  <td data-title="中文法規名稱" class="num">積體電路電路布局保護法</td>
                  <td data-title="更新日期" class="num">民國106年6月12日</td>
                </tr>
                <tr>
                  <td data-title="編號" class="num">07</td>
                  <td data-title="修正日期" class="num">民國91年6月12日</td>
                  <td data-title="中文法規名稱" class="num">積體電路電路布局保護法</td>
                  <td data-title="更新日期" class="num">民國106年6月12日</td>
                </tr>
                <tr>
                  <td data-title="編號" class="num">08</td>
                  <td data-title="修正日期" class="num">民國91年6月12日</td>
                  <td data-title="中文法規名稱" class="num">積體電路電路布局保護法</td>
                  <td data-title="更新日期" class="num">民國106年6月12日</td>
                </tr>
                <tr>
                  <td data-title="編號" class="num">09</td>
                  <td data-title="修正日期" class="num">民國91年6月12日</td>
                  <td data-title="中文法規名稱" class="num">積體電路電路布局保護法</td>
                  <td data-title="更新日期" class="num">民國106年6月12日</td>
                </tr>
                <tr>
                  <td data-title="編號" class="num">10</td>
                  <td data-title="修正日期" class="num">民國91年6月12日</td>
                  <td data-title="中文法規名稱" class="num">積體電路電路布局保護法</td>
                  <td data-title="更新日期" class="num">民國106年6月12日</td>
                </tr>
              </tbody>
            </table>
          </div>
        </section>
        <!-- LP End -->
        <!-- Pagination -->
        <?php require_once('include/pagination.html'); ?>
      </div>
    </div>
    <!-- 訂閱電子報 -->
    <?php require_once('include/subscribe.html'); ?>
    <!-- fatfooter Start -->
    <?php require_once('include/fatfooter.html'); ?>
    <!-- fatfooter End -->
    <!-- footer Start-->
    <?php require_once('include/footer.html'); ?>
    <!-- footer End -->
  </div>
  <a href="javascript:;" class="scrollToTop">回頁首</a>
  <!-- JQ -->
  <script src="js/jquery-3.3.1.min.js"></script>
  <!-- plugin js -->
  <script src="vendor/jquery.easing.min.js"></script>
  <script src="vendor/slick/slick.min.js "></script>
  <script src="vendor/slick/slick-lightbox.js "></script>
  <script src="vendor/lazyload/lazyload.min.js"></script>
  <!-- nicescroll -->
  <script src="vendor/nicescroll/jquery.nicescroll.min.js"></script>
  <!-- hyUI -->
  <script src="js/hyui.js"></script>
  <!-- 客製js -->
  <script src="js/customize.js"></script>
</body>

</html>